...
| Critical | Demo quote form opened an unverified email address and did not submit online. | Replaced with a FormSubmit POST to the user-approved `markoplixo@gmail.com`, spam check, honeypot, privacy notice and confirmation page; phone fallback remains. | **Activation required:** owner must submit a test, confirm the activation email, then submit and receive a second test. |
...
1. **Activate the approved quote inbox:** user confirmed `markoplixo@gmail.com`; FormSubmit must send a first-use activation email and the owner must approve it. Then submit a second test and verify receipt before promoting online quote submissions.
